...
팰린드롬의 시작과 끝에 같은 숫자를 추가하면 팰린드롬이다. 이 아이디어에서 착안하여, 다음과 같은 점화식을 세울 수 있다.<br>
dp[st][ed] = (dp[st + 1][ed - 1] == 1 && num[st] == num[ed]) ? 1 : 0<br>
이 dp를 돌리기 위해서 길이가 1, 2인 수열에 대해 따로 처리해 주어야 한다.<br>
길이가 1인 수열자체는 펠린드롬이고, 길이가 2인 수열은 그냥 두 숫자가 같으면 펠린드롬이므로 이를 입력시에 처리해주면 된다.

### 성능 요약

메모리: 16780 KB, 시간: 352 ms
