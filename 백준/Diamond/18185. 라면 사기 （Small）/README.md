...
처음에 그냥 3개 > 2개 > 1개순으로 처리하는 greedy로 풀었는데, WA가 나왔다. </br>
WA를 받고 생각해보니, ramen[i + 1] > ramen[i + 2]일땐 2개를 먼저 처리하는것이 유리함을 깨달았고, (ramen[i + 1] - ramen[i + 2])만큼 2개 짜리를 먼저 처리하게 수정하였다.<br/>
